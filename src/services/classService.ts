...
export class ClassService {
	...
	static async getClassTemplates(filters: {
		...
		if (filters.discipline) {
			const discipline = await ClassDiscipline.findOne({
				slug: filters.discipline,
			});
			if (discipline) {
	...
	static async getClassTemplateBySlug(slug: string): Promise<any | null> {
		return ClassTemplate.findOne({ slug })
			.populate("disciplineId", "name slug")
			.populate("coachIds", "name photo specialties");
	}
}
